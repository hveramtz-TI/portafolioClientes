...
**Fecha:** 2026-08-28
**Estado:** 🔵 **En progreso (actualizado 2026-09-21)** — Slices 1–5 (de 7) integrados en `main`, verificados y cerrados (cambios SDD archivados). Pendientes: Slices 6–7 (frontend de catálogo) y transiciones end-to-end HU-013–HU-025. El detalle en *Registro de progreso* al final.
...
| D5 | Unicidad | **Base:** `name` único global (rubros), `rubro_id + name` (categorías), `categoria_id + title` (services). **Personal:** identidad única `user_id + item_type + base_id` — validación de aplicación (primera línea) **+ índice único parcial en DB** `(user_id, item_type, base_id) WHERE base_id IS NOT NULL AND deleted_at IS NULL` (migración `2026_09_14_000000_add_user_catalog_live_identity_unique_index`, reversible y verificada en SQLite y PostgreSQL; **enmienda aprobada por el maintainer** en Slice 4): un race que supere la validación se renderiza 409 con el mismo contrato de error; nombre/título visible único dentro de `parent_fork_id` (se mantiene solo en aplicación — no indexable cross-engine). No se usan FKs ni índices únicos genéricos para lo polimórfico | HU-013/014/017/018/021/022/025, flujos CRUD |
...
### 2026-09-14 — Slice 4 (API HTTP de forks) — Unidades 4a y 4b

Cambio SDD `catalog-slice-4-fork-api` (proposal/spec/design/tasks/apply-progress bajo `openspec/changes/catalog-slice-4-fork-api`; 7 requisitos R1–R7 más el delta de personalización con S8). Dos unidades encadenadas:

- **4a — CRUD passthrough, contratos, índice** (`1d78d8f`, rama `feat/catalog-slice-4a-fork-api`): superficie HTTP `/api/user-catalog/*` bajo `auth:sanctum` fuera de `role:admin`; fork en cascada, store personal, show resuelto, update con overrides mergeados, endpoints dedicados deactivate/reactivate; claves estructurales (`item_type`, `parent_fork_id`, `sort_order`) expuestas por el resolver; política `create` owner-only; **índice único parcial** (enmienda D5) reversible; mapa único de errores 409. 23 escenarios feature (S1–S3, S7).
- **4b — Árbol, cascada, mover/adjuntar** (`d2837bc`, rama `feat/catalog-slice-4b-tree-cascade-move`): `GET /api/user-catalog/tree` con anidado rubro→categorías→servicios, filtros `status` (**estado efectivo**) y `origin`, orden por `sort_order` y presupuesto de consultas constante (medido 4, asertado ≤6); `DELETE` con **soft-delete en cascada** owner-scoped en una transacción (bases y filas ajenas intactas; re-fork tras borrar es legal); guardas completas de move/attach (coherencia de tipo, ownership, ciclo de descendientes, unicidad de nombre visible en destino). 19 escenarios feature (S4–S6, S8).

**Verificación:** suite completa **214 tests / 771 aserciones** en verde en SQLite **y** PostgreSQL (baseline previa: 195); Pint PASS en los 8 archivos tocados; CodeGraph sincronizado. Los 20 escenarios del Slice 3 y los 23 de la Unit 1 siguen verdes.

**Enmienda D5 registrada** (fila D5): la identidad de fork ahora también se refuerza con el índice único parcial en DB (validación de aplicación como primera línea; el race se renderiza 409); la unicidad de nombre visible entre hermanos queda solo en aplicación (no indexable cross-engine).
