...
### Setup
- [ ] 1.1 Acquire ledger token for Unit 1: `gentle-ai sdd-attempt acquire --cwd /home/hgvm/Documentos/GitHub/portafolioClientes --change catalog-slice-4-fork-api --request-id u1-crud --work-unit u1-crud --evidence-goal "R1,R2,R3,R7,S1-S3,S7 green on SQLite+PG" --max-attempts 3 --max-changed-lines 400`

### RED: Write failing tests for Unit 1
- [ ] 1.2 RED: Create `backend/tests/Feature/UserCatalogForkApiCrudTest.php` covering S1.1 (rubro cascade summary), S1.2 (categoria cascade), S1.3 (duplicate root 409), S1.4 (duplicate descendant 409), S1.5 (unknown base 404), S1.6 (fork deactivated base)
- [ ] 1.3 RED: Add tests for S2.1 (store personal), S2.2 (store base_id rejected 422), S2.3 (show resolved with structural keys), S2.4 (update null removes override), S2.5 (status/unknown field 422), S2.6 (J4 rubro null parent legal), S2.7 (rubro non-null parent 422)
- [ ] 1.4 RED: Add tests for S3.1 (deactivate hides subtree), S3.2 (reactivate only selected), S3.3 (reactivate below dead ancestor), S3.4 (status via update rejected)
- [ ] 1.5 RED: Add tests for S7.1 (401 unauthenticated), S7.2 (403 cross-owner), S7.3 (404 missing/soft-deleted), S7.4 (409 duplicate service path), S7.5 (409 index race path both engines), S7.6 (422 validation)

### GREEN: Implement Unit 1 infrastructure
- [ ] 1.6 GREEN: Create migration `backend/database/migrations/XXXX_XX_XX_XXXXXX_add_live_identity_unique_index.php` with reversible raw SQL partial unique index `(user_id,item_type,base_id) WHERE base_id IS NOT NULL AND deleted_at IS NULL`; `down()` drops it
- [ ] 1.7 GREEN: Add routes to `backend/routes/api.php` for all 7 endpoints under `auth:sanctum` outside `role:admin` with `{type}` constraint `rubros|categorias|services`
- [ ] 1.8 GREEN: Create `backend/app/Http/Controllers/UserCatalog/ForkController.php` — `fork()` delegates to `CascadeForkService`, normalizes summary, returns 201
- [ ] 1.9 GREEN: Create `backend/app/Http/Controllers/UserCatalog/StoreController.php` — `store()` validates route type, creates personal item (`base_id` null, `status` activo), rejects non-null `base_id` 422
- [ ] 1.10 GREEN: Create `backend/app/Http/Controllers/UserCatalog/ShowController.php` — `show()` returns resolved view with `item_type`, `parent_fork_id`, `sort_order`
- [ ] 1.11 GREEN: Create `backend/app/Http/Controllers/UserCatalog/UpdateController.php` — `update()` merges overrides (explicit null removes), rejects `status`/unknown fields 422, rubro `parent_fork_id: null` legal, non-null 422
- [ ] 1.12 GREEN: Create `backend/app/Http/Controllers/UserCatalog/StatusController.php` — `deactivate()`/`reactivate()` write only own `status`, return resolved view; reactivate touches only selected item (D7)
- [ ] 1.13 GREEN: Create `backend/app/Http/Controllers/UserCatalog/DeleteController.php` — `destroy()` calls `CascadeDeleteUserCatalogService`, returns 204
- [ ] 1.14 GREEN: Create `backend/app/Http/Controllers/UserCatalog/TreeController.php` — `tree()` validates `status|origin` filters, delegates to `UserCatalogTreeService`, returns nested graph
- [ ] 1.15 GREEN: Modify `backend/app/Http/Requests/StoreUserCatalogItemRequest.php` — add route-type coherence, reject non-null `base_id` with fork-endpoint message, allow `parent_fork_id: null` for rubro only
- [ ] 1.16 GREEN: Modify `backend/app/Http/Requests/UpdateUserCatalogItemRequest.php` — add `parent_fork_id` as accepted update key with type-coherence/ownership/cycle guards, reject `status`/unknown fields
- [ ] 1.17 GREEN: Modify `backend/app/Policies/UserCatalogItemPolicy.php` — add `create()` ability (owner only, admin denied)
- [ ] 1.18 GREEN: Modify `backend/bootstrap/app.php` — add `withExceptions` map: `DomainException` + `QueryException` (PG 23505, SQLite unique) → identical 409 JSON `{message}`

### REFACTOR: Clean up Unit 1
- [ ] 1.19 REFACTOR: Extract shared controller base if duplication > 15 lines; ensure Pint passes
- [ ] 1.20 REFACTOR: Verify migration runs clean on both engines; `php artisan migrate:fresh --seed` green

### VERIFY: Unit 1 dual-engine + regression
- [ ] 1.21 VERIFY: Run `docker compose exec backend php artisan test --filter=UserCatalogForkApiCrudTest` (SQLite)
- [ ] 1.22 VERIFY: Run `./test-pg.sh --filter=UserCatalogForkApiCrudTest` (PostgreSQL)
- [ ] 1.23 VERIFY: Run `docker compose exec backend php artisan test --filter=CatalogResolverTest,CascadeForkServiceTest,UserCatalogItemPolicyTest,UserCatalogItemRequestTest` (Slice 3 regression, both engines)
- [ ] 1.24 VERIFY: Run `docker compose exec backend ./vendor/bin/pint --test`
- [ ] 1.25 Commit work unit: `feat(catalog): add fork HTTP API — CRUD, contracts, index (R1,R2,R3,R7)` with tests
