...
| D-5 | `UserCatalogTreeService` eager-loads owner roots and bounded `children.base` paths through maximum depth, then resolves/filters/orders in memory. | O(depth), not O(nodes); filters add no queries and preserve retained nesting. |
